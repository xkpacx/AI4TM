...
- **When editing `.ipynb` files directly (not through a notebook tool), check first whether the file's non-ASCII characters (em-dashes, curly quotes, etc.) are stored as literal UTF-8 or as `\uXXXX` escapes** (`grep -c '\\u2014'` against a known em-dash spot is a quick check). If it's `\uXXXX`-escaped, `json.load` then modify the Python string value then `json.dump(nb, f, indent=1, ensure_ascii=False)` will silently re-escape *every* such character across the *whole file* into literal UTF-8, producing a large, unrelated-looking diff (this happened mid-session and had to be reverted). In that case use targeted raw string substitution instead (find the exact bytes around the insertion point, replace, `json.load()` the result to confirm it's still valid). Where the file already uses literal UTF-8 (no `\uXXXX` escapes to begin with), `json.load`/`dump` is fine and preserves formatting well, and it sidesteps the backslash-escaping mistakes raw substitution is prone to (`\\\\$` in a non-raw Python string literal to produce a decoded `\$`, easy to get wrong and silently write invalid JSON). Either way: if a batch of substitutions is written as "collect all, assert none missing, write," a single missing pattern silently discards the *entire* batch, not just the one that didn't match, so re-verify with a fresh read after any such script reports success.
