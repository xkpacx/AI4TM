# Hands-on: build a knowledge graph for internal documentation
...
> That's the actual mechanism behind "a graph improves factual accuracy": a graph turns a fact-assembly step the model would otherwise have to infer from raw text into a deterministic database lookup performed before the model is ever asked to reason at all, not a matter of the graph making the model itself smarter.
